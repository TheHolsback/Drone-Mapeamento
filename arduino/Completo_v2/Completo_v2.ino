...
bool readLD06Packet() {
  // Estado persiste entre chamadas — nunca bloqueia o loop
  static uint8_t localBuf[PACKET_SIZE];
  ...
  while (lidarSerial.available()) {
    ...
    if (!synced) {
      ...
      // Valida o 2º byte o mais cedo possível para evitar falso 0x54
      if (idx == 2) {
        if (localBuf[1] != 0x2C) {
          // Falso início — verifica se o byte atual pode ser um novo início
          if (localBuf[1] == 0x54) {
            localBuf[0] = 0x54;
            idx = 1;                // reinicia com este byte como candidato
          } else {
            synced = false;
            idx    = 0;
          }
      ...
      // Pacote completo coletado
      if (idx == PACKET_SIZE) {
        memcpy(packet, localBuf, PACKET_SIZE);
        synced = false;
        idx    = 0;
        return true;
      }
  ...
  return false; // ainda não há pacote completo — volta no próximo loop
}
